...
`ReportingService` accepts an optional `report_sink: ReportSink | None`
constructor parameter, preserving full backwards compatibility. When a sink is
...
the `Repository` record for `github_owner` and `github_name`, calls
`render_report_markdown()`, and delegates to the sink.
