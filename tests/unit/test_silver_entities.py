...
@pytest.mark.asyncio
async def test_commit_event_creates_repo_and_commit(
...
@pytest.mark.asyncio
async def test_pull_request_events_update_existing_record(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    ...
    async with session_factory() as session:
        ...
        repo_count = await session.scalar(select(func.count()).select_from(Repository))
        assert repo_count == 1


@pytest.mark.asyncio
async def test_issue_event_creates_issue(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
...
@pytest.mark.asyncio
async def test_documentation_change_creates_stub_commit_when_missing(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
...
@pytest.mark.asyncio
async def test_documentation_change_is_upserted_by_commit_and_path(
    session_factory: async_sessionmaker[AsyncSession],
) -> None:
    ...
    async with session_factory() as session:
        ...
        assert doc_change.occurred_at == dt.datetime(2024, 7, 5, 13, 56, tzinfo=dt.UTC)
