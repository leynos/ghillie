...
class TestOpenAIResponseExtraction:
    ...
    def test_extract_content_shape_errors(
        self,
        model: OpenAIStatusModel,
        response_data: ResponseData,
        expected_message: str,
    ) -> None:
        """Malformed response shapes raise field-specific shape errors."""
        with pytest.raises(OpenAIResponseShapeError, match=expected_message):
