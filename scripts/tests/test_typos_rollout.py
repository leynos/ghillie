...
def test_local_policy_preserves_repository_exceptions(
    rollout_modules: tuple[types.ModuleType, types.ModuleType, types.ModuleType],
    tmp_path: Path,
) -> None:
    """The generated config retains the committed local spelling policy."""
    ...
    assert config["default"]["extend-words"]["ANALYZE"] == "ANALYZE", (
        "repository spelling-policy exception must survive generated configuration"
    )
